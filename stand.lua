...
local function showBootBanner()
    pcall(function()
        ...
        local guiParent = lp:FindFirstChildOfClass("PlayerGui") or lp:WaitForChild("PlayerGui", 5)
        if not guiParent then
            return
        end
        local gui = Instance.new("ScreenGui")
        gui.Name = "VN_BOOT_BANNER"
...
task.defer(showBootBanner)
pcall(function()
    warn("[V!N] Loaded")
end)

local Owner = "Mahdirml123i"
local AllowLocalCommands = true
local ForceLocalOwner = true
local AutoStart = true
local AutoStartDelay = 0.5
local ShowLoadNotification = true
...
pcall(function()
    textChannel = TextChatService.TextChannels and TextChatService.TextChannels:FindFirstChild("RBXGeneral")
end)
pcall(function()
...
local function showLoadNotification()
    if not ShowLoadNotification then
        return
    end
    pcall(function()
...
local legacyChatConnections = {}
local recentChat = {}

local TextChatService = game:GetService("TextChatService")

function isAuthorized(player)
    if AllowLocalCommands then
...
function removeOldListeners()
    ...
    for userId, conn in pairs(legacyChatConnections) do
        if conn then
            conn:Disconnect()
        end
        legacyChatConnections[userId] = nil
    end
    recentChat = {}
end

local function shouldSkipDuplicate(player, message)
    local now = os.clock()
    local record = recentChat[player]
    if record and record.text == message and (now - record.time) < 0.25 then
        return true
    end
    recentChat[player] = {text = message, time = now}
    return false
end
...
function setupChatListener(player)
    ...
    activeListeners[player.UserId] = function(msg)
        ...
        local message = tostring(msg.Text or "")
        if message == "" then
            return
        end
        if shouldSkipDuplicate(player, message) then
            return
        end
        ...
        if (isAuthorized(player)) then
            ...
            elseif msgLower:match("^%.tp%s+(.+)$") then
                ...
                hardStop = false
                local destination = msgLower:match("^%.tp%s+(.+)$")
                destination = trimInput(destination)
                if destination and destination ~= "" then
                    handleGotoCommand(player.Name, destination)
                end
    ...
    if not legacyChatConnections[player.UserId] then
        legacyChatConnections[player.UserId] = player.Chatted:Connect(function(message)
            local handler = activeListeners[player.UserId]
            if handler then
                handler({Text = message, TextSource = {UserId = player.UserId}})
            end
        end)
    end
end
...
activatePowerMode = function()
    ...
    -- POWER MODE: maximum aggressiveness (higher fire rate / faster loops)
    LowLagMode = false
    perf.loop = 0.005
    perf.combat = 0.005
    perf.void = 0.03
    ...
    -- more bullets per tick (normal + follow)
    shotsPerTick = 25
    followShotsPerTick = 12
    followShotCooldown = 0.008

    -- wider scan + more targets
    followGridRadius = 450
    followMaxTargets = 25

    -- faster reload attempts
    reloadCooldown = 0.1
    ...
    sendMessage("POWER MODE enabled.")
end
...
if not getgenv()._standworks_injection then
    getgenv()._standworks_injection = true

    getgenv().allowVehicleSeats = true
    getgenv().autoSeatOwner = true
    getgenv().tpCommandAlias = false
    getgenv().extraLocations = getgenv().extraLocations or {
        gs = Vector3.new(415.067, 76.778, 1.685),
        gunstore = Vector3.new(415.067, 76.778, 1.685),
        gun = Vector3.new(415.067, 76.778, 1.685),
    }
    ...
    local function isSeatInstance(inst)
        return typeof(inst) == "Instance" and inst:IsA("Seat")
    end

    if not getgenv()._seatDestroyHooked then
        getgenv()._seatDestroyHooked = true
        pcall(function()
            if hookfunction and Instance and Instance.Destroy then
                local oldDestroy
                oldDestroy = hookfunction(Instance.Destroy, function(self, ...)
                    if getgenv().allowVehicleSeats and isSeatInstance(self) then
                        return
                    end
                    return oldDestroy(self, ...)
                end)
                getgenv()._seatDestroyOld = oldDestroy
            end
        end)
        pcall(function()
            if hookmetamethod and getnamecallmethod then
                ...
                oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                    ...
                    if getgenv().allowVehicleSeats and method == "Destroy" and isSeatInstance(self) then
                        return
                    end
                    return oldNamecall(self, ...)
                end)
                getgenv()._seatDestroyOldNamecall = oldNamecall
            end
        end)
    end

    if handleGotoCommand then
        getgenv()._origHandleGotoCommand = getgenv()._origHandleGotoCommand or handleGotoCommand
    end

    handleGotoCommand = function(playerName, locationName)
        local Players = game:GetService("Players")
        local player = Players:FindFirstChild(playerName)
        if not player then return end

        locationName = tostring(locationName or ""):lower()
        if locationName == "" then return end
        ...
        for name, pos in pairs(getgenv().extraLocations or {}) do
            local key = tostring(name):lower()
            if typeof(pos) == "CFrame" then
                locationCFrames[key] = pos
            elseif typeof(pos) == "Vector3" then
                locationCFrames[key] = CFrame.new(pos)
            end
        end
    ...
    if activeListeners and not activeListeners._tpAlias then
        activeListeners._tpAlias = function(msg)
            if not getgenv().tpCommandAlias then return end
            local sender = msg.TextSource and msg.TextSource.UserId and Players:GetPlayerByUserId(msg.TextSource.UserId)
            if not sender or sender.Name ~= Owner then return end
            local message = tostring(msg.Text or "")
            local msgLower = message:lower()
            local destination = msgLower:match("^%.tp%s+(.+)$")
            if destination then
                ...
                if destination ~= "" then
                    handleGotoCommand(sender.Name, destination)
                end
            end
        end
    end
    ...
    task.spawn(function()
        while task.wait(0.1) do
            if not getgenv().autoSeatOwner then
                continue
            end
            if not (vehicleMode and gotoPlayer and (gotoCFrame or gotoTarget)) then
                continue
            end
            ...
            local ownerHumanoid = ownerChar and ownerChar:FindFirstChildOfClass("Humanoid")
            local ownerHRP = ownerChar and ownerChar:FindFirstChild("HumanoidRootPart")
            if not (ownerHumanoid and ownerHRP) then
                continue
            end

            local vehicle = findVehicleModel and findVehicleModel()
            if not vehicle then
                continue
            end

            local driverSeat, passengerSeat = getVehicleSeats and getVehicleSeats(vehicle)
            ...
            if passengerSeat.Occupant ~= ownerHumanoid then
                local vehicleRoot = driverSeat or (getVehicleRootPart and getVehicleRootPart(vehicle))
                if vehicleRoot and moveVehicleRootToSeat then
                    local desiredSeatCFrame = ownerHRP.CFrame * CFrame.new(0, 0, 1.5)
                    moveVehicleRootToSeat(vehicleRoot, passengerSeat, desiredSeatCFrame)
                end
                pcall(function()
