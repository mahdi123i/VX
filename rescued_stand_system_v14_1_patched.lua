...
local function SafeLoadExternal(url)
    ...
    if not httpSuccess then
        httpGetError = "HttpGet call failed (executor may not support it)"
        return false, httpGetError
    end
    
    if sourceCode == nil then
        httpGetError = "HttpGet returned nil (network error or blocked)"
        return false, httpGetError
    end
    
    if type(sourceCode) ~= "string" then
        httpGetError = "HttpGet returned non-string: " .. type(sourceCode)
        return false, httpGetError
    end
    
    if sourceCode == "" then
        httpGetError = "HttpGet returned empty string"
        return false, httpGetError
    end
    
    print("[BOOTSTRAP] ✓ HttpGet returned valid source code (" .. #sourceCode .. " bytes)")
    
    -- STEP 2: Verify loadstring returns a FUNCTION
    local loaderFunc = nil
    local loadstringError = nil
    ...
    if not loadSuccess then
        loadstringError = "loadstring call failed (syntax error in source)"
        return false, loadstringError
    end
    
    if loaderFunc == nil then
        loadstringError = "loadstring returned nil (invalid Lua syntax)"
        return false, loadstringError
    end
    
    if type(loaderFunc) ~= "function" then
        loadstringError = "loadstring returned non-function: " .. type(loaderFunc)
        return false, loadstringError
    end
    
    print("[BOOTSTRAP] ✓ loadstring returned valid function")
    ...
    if not execSuccess then
        execError = "External function execution failed"
        return false, execError
    end
    
    print("[BOOTSTRAP] ✓ External function executed successfully")
    return true, nil
end
...
local function InitializeExternalBootstrap()
    ...
    -- RESPECT executor's pre-set config (do NOT override)
    ...
    -- Merge executor's _C config with defaults (executor config takes priority)
    local defaultConfig = {
        Fps = false,
        Msg = "Yare Yare Daze.",
        AntiMod = true,
    ...
    if not getgenv()._C then
        ...
        -- Merge: keep executor's values, fill missing with defaults
    ...
    -- SAFE EXTERNAL LOADER CALL
    local loaderUrl = ""
    if loaderUrl and loaderUrl ~= "" then
        bootstrapSuccess, bootstrapError = SafeLoadExternal(loaderUrl)
    else
...
function RemoteAnalyzer:HookRemote(remoteName, remoteObj)
    ...
    pcall(function()
        originalFire = remoteObj.FireServer
    end)
    
    if not originalFire or typeof(originalFire) ~= "function" then
        return
    end
